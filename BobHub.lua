...
if e then
	oldwarnmessage("BOBHUB v"..currentver, "Failed to get script data.", 10)
end

if BOBHUBLOADED == true then warnmessage("BOBHUB v"..currentver, "GUI already loaded!", "", 10) return end
if game.PlaceId ~= 6839171747 and game.PlaceId == 6516141723 then 
	...
	confirmnotification("BOBHUB v"..currentver, "Do you want to join a game?", 15, function(state)
		if state == true then
			task.spawn(function()
...
if game.PlaceId ~= 6839171747 and game.PlaceId ~= 6516141723 then 
	oldwarnmessage("BOBHUB v"..currentver, "You need to join DOORS to run this script.", 10) 
	return
end
if gui_data ~= nil then
	if currentver ~= gui_data.ver or gui_data.ver ~= currentver then
		warnmessage("BOBHUB v"..currentver, "You are using an outdated version of this script", "Loading latest version.", 10) 
...
normalmessage("BOBHUB v"..currentver, "Loading script...", "", 2)
if gui_data ~= nil then
...
local Window = Library:CreateWindow({
    ...
    Title = 'BOBHUB v'..currentver..' | DOORS 👁️',
    Center = true,
    AutoShow = true,
    Resizable = true,
...
LeftGroupBox:AddToggle('MyToggle', {
	...
	warnmessage("BOBHUB v"..currentver, "You need to have fireproximityprompt function for 'instant use'.", 7)
end
...
game:GetService("TextChatService").TextChannels.RBXGeneral:SendAsync("Eyes Has Spawn Don't Look At him")
...
oldwarnmessage("BOBHUB v"..currentver, "You need to have fireproximityprompt function for 'loot aura'.", 7)
end
...
local Collision = game.Players.LocalPlayer.Character.Collision
    ...
    firesignal(game.ReplicatedStorage.EntityInfo.Caption.OnClientEvent, "GodMode is Enable...")
task.spawn(function()
...
local Collision = game.Players.LocalPlayer.Character.Collision
    Collision.Position = Collision.Position + Vector3.new(0,9.8,0)
    firesignal(game.ReplicatedStorage.EntityInfo.Caption.OnClientEvent, "GodMode is Disable...")
task.spawn(function()
		local notif = Instance.new("Sound");notif.Parent = game.SoundService;notif.SoundId = "rbxassetid://6897623656";notif.Volume = 2;notif:Play();notif.Stopped:Wait();notif:Destroy()
		end)
end
...
LeftGroupBox4:AddLabel('a00pkidd, Wvpul, yieviro92')
...
local MyButton = LeftGroupBox4:AddButton({
    ...
    Func = function()
        if setclipboard then 
            setclipboard("youtube.com/@a00pkidd")   
        end
    end,
    DoubleClick = false,
    Tooltip = ''
...
local MyButton = LeftGroupBox4:AddButton({
    ...
    Func = function()
        if setclipboard then 
            setclipboard("https://youtube.com/@kinghub9999")   
        end
    end,
    DoubleClick = false,
    Tooltip = ''
...
local MyButton = LeftGroupBox4:AddButton({
    ...
    Func = function()
        if setclipboard then 
            setclipboard("youtube.com/@wvpul")   
        end
    end,
    DoubleClick = false,
    Tooltip = ''
...
local MyButton = LeftGroupBox4:AddButton({
    Text = 'Deleted Users#8698 [KingHub]',
    Func = function()
        if setclipboard then 
            setclipboard("Deleted Users#8698")   
        end
    end,
    DoubleClick = false,
    Tooltip = ''
