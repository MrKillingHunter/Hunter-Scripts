...
local venyx = library.new("Hunter Library (v1.1) - Arsenal", 5013109572)

local themes = {
   Background = Color3.fromRGB(24, 24, 24),
   Glow = Color3.fromRGB(0, 255, 224),
...
local Furry_Chatspam = {'UwU Sowwiez! Im using HL and owning youwww </3', 'OwO hameware is tapping aww of youw fwiends :c', 'e.e you arent using hl? Dat makes meh angerye!! >:c', '>.< I am tapping eyes closed with hl~', ':3 come get hl! discoard.gg/'..invitecode, 'OvO hameware is slotted!! C:', 'UvU no challenge for hameware! </3'}
...
local section3 = Visuals:addSection("ESP")
...
section3:addToggle("Highlight ESP", nil, function(value)
   Chams_Toggled = value
end)

section3:addColorPicker('ESP Color', Color3.fromRGB(50, 50, 50), function(color3)
   ChamsColor = color3
end)
...
addonsectionVISUALS:addToggle("Arm ESP", nil, function(value)
   ArmChams = value
end)

addonsectionVISUALS:addToggle('Weapon ESP', nil, function(value)
   WeaponChams = value
end)
...
Movement:addToggle("Infinte Jump", nil, function(value)
   Bhop_Toggled = value
end)

Movement:addSlider("Inf Jump Speed", 1, 0, 100, function(value)
   Bhop_Speed = value
end)
...
Troll:addDropdown("Chatspam Type", {"Furry", "Swiss", "HvH", "China"}, function(text)
   Chatspam_Type = text
end)
...
local theme = venyx:addPage("Configs", 5012543246)
local colors = theme:addSection("Colors")
local uitogl = theme:addSection("GUI")

uitogl:addKeybind("UI Toggle Keybind", Enum.KeyCode.End, function(val)
   if uion then
